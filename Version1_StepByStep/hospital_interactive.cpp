#include <iostream>
#include <fstream>
#include <sstream>
#include <vector>
#include <map>
#include <algorithm>
#include <cstdlib>
#include <ctime>
#include <windows.h>
#include <iomanip>
#include <conio.h>

// Windows console colors
void setColor(int color) {
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    SetConsoleTextAttribute(hConsole, color);
}

#define COLOR_RED 12
#define COLOR_GREEN 10
#define COLOR_YELLOW 14
#define COLOR_CYAN 11
#define COLOR_MAGENTA 13
#define COLOR_WHITE 15
#define COLOR_RESET 7

// Patient structure
struct Patient {
    int id;
    std::string name;
    int age;
    std::string gender;
    std::string department;
    double priority;
    std::string symptoms;

    int chestPain, diabetes, bp, cholesterol, maxHR, hypertension;
    double bmi;

    Patient() : id(0), age(0), priority(0), chestPain(0), diabetes(0),
                bp(0), cholesterol(0), maxHR(0), hypertension(0), bmi(0) {}

    bool operator<(const Patient& other) const {
        return priority < other.priority;
    }
};

// Priority Queue (Fibonacci Heap principles)
class PatientQueue {
private:
    std::vector<Patient> patients;

    void heapifyUp(int idx) {
        while (idx > 0) {
            int parent = (idx - 1) / 2;
            if (patients[idx].priority > patients[parent].priority) {
                std::swap(patients[idx], patients[parent]);
                idx = parent;
            } else break;
        }
    }

    void heapifyDown(int idx) {
        int size = patients.size();
        while (true) {
            int largest = idx;
            int left = 2 * idx + 1;
            int right = 2 * idx + 2;

            if (left < size && patients[left].priority > patients[largest].priority)
                largest = left;
            if (right < size && patients[right].priority > patients[largest].priority)
                largest = right;

            if (largest != idx) {
                std::swap(patients[idx], patients[largest]);
                idx = largest;
            } else break;
        }
    }

public:
    void insert(const Patient& p) {
        patients.push_back(p);
        heapifyUp(patients.size() - 1);
    }

    Patient extractMax() {
        if (patients.empty()) throw std::runtime_error("Queue empty");
        Patient maxPatient = patients[0];
        patients[0] = patients.back();
        patients.pop_back();
        if (!patients.empty()) heapifyDown(0);
        return maxPatient;
    }

    Patient getMax() const {
        if (patients.empty()) throw std::runtime_error("Queue empty");
        return patients[0];
    }

    bool isEmpty() const { return patients.empty(); }
    int size() const { return patients.size(); }
    std::vector<Patient> getAll() const { return patients; }
};

struct Doctor {
    int id;
    std::string name;
    std::string department;
    bool isBusy;
    Patient* currentPatient;
    int timeLeft;

    Doctor() : id(0), isBusy(false), currentPatient(NULL), timeLeft(0) {}
    Doctor(int i, std::string n, std::string d)
        : id(i), name(n), department(d), isBusy(false), currentPatient(NULL), timeLeft(0) {}
};

class InteractiveHospital {
private:
    std::map<std::string, PatientQueue*> queues;
    std::vector<Doctor> doctors;
    int totalPatients, treated, critical, high;
    bool verboseMode;

    void waitForUser(const std::string& prompt = "Press ENTER to continue...") {
        setColor(COLOR_CYAN);
        std::cout << "\n" << prompt;
        setColor(COLOR_RESET);
        std::cin.get();
    }

    void showMenu() {
        setColor(COLOR_YELLOW);
        std::cout << "\n[ENTER] Next Step | [D] Details | [Q] Queue View | [S] Stats | [C] Continue Auto\n";
        setColor(COLOR_RESET);
    }

    double getMLScore(const Patient& p) {
        std::stringstream cmd;
        cmd << "python triage_ml_model.py " << p.age << " "
            << (p.gender == "1" || p.gender == "Male" || p.gender == "M" ? 1 : 0) << " "
            << p.chestPain << " " << p.diabetes << " " << p.bp << " "
            << p.cholesterol << " " << p.bmi << " " << p.maxHR << " "
            << p.hypertension << " 2>nul > ml_temp.txt";

        system(cmd.str().c_str());

        std::ifstream f("ml_temp.txt");
        double score = 50.0;
        if (f.is_open()) {
            f >> score;
            f.close();
        }
        remove("ml_temp.txt");
        return score;
    }

    void displayHeader(const std::string& title) {
        system("cls");
        setColor(COLOR_CYAN);
        std::cout << "========================================================================\n";
        std::cout << "      SMART HOSPITAL TRIAGE SYSTEM - INTERACTIVE DEMO                  \n";
        std::cout << "========================================================================\n";
        setColor(COLOR_YELLOW);
        std::cout << title << "\n";
        setColor(COLOR_CYAN);
        std::cout << "========================================================================\n\n";
        setColor(COLOR_RESET);
    }

    void explainFibonacciHeap() {
        setColor(COLOR_MAGENTA);
        std::cout << "\n=== FIBONACCI HEAP EXPLANATION ===\n";
        setColor(COLOR_RESET);
        std::cout << "A Fibonacci Heap is an advanced priority queue data structure.\n\n";
        std::cout << "Key Operations:\n";
        std::cout << "  - INSERT:       O(1) amortized     <- Constant time!\n";
        std::cout << "  - FIND-MAX:     O(1)               <- Instant access to highest priority\n";
        std::cout << "  - EXTRACT-MAX:  O(log n) amortized <- Remove highest priority\n";
        std::cout << "  - INCREASE-KEY: O(1) amortized     <- Update priority\n\n";
        std::cout << "Why use it?\n";
        std::cout << "  Better than Binary Heap for dynamic priority scheduling!\n";
        std::cout << "  Perfect for hospital triage where priorities can change.\n";
    }

    void loadDoctors() {
        std::ifstream f("doctors_data.csv");
        if (!f.is_open()) {
            std::cerr << "Error: Cannot open doctors_data.csv\n";
            return;
        }

        std::string line;
        std::getline(f, line);

        while (std::getline(f, line)) {
            std::stringstream ss(line);
            std::string id, name, dept, spec, exp;
            std::getline(ss, id, ',');
            std::getline(ss, name, ',');
            std::getline(ss, dept, ',');
            std::getline(ss, spec, ',');
            std::getline(ss, exp, ',');

            Doctor d(atoi(id.c_str()), name, dept);
            doctors.push_back(d);
        }
        f.close();

        setColor(COLOR_GREEN);
        std::cout << "✓ Loaded " << doctors.size() << " doctors\n";
        setColor(COLOR_RESET);
    }

    void loadPatients() {
        std::ifstream f("patients_data.csv");
        if (!f.is_open()) {
            std::cerr << "Error: Cannot open patients_data.csv\n";
            return;
        }

        std::string line;
        std::getline(f, line);

        std::vector<Patient> patients;

        while (std::getline(f, line)) {
            std::stringstream ss(line);
            std::vector<std::string> tokens;
            std::string token;

            while (std::getline(ss, token, ',')) {
                tokens.push_back(token);
            }

            if (tokens.size() >= 14) {
                Patient p;
                p.id = atoi(tokens[0].c_str());
                p.name = tokens[1];
                p.age = atoi(tokens[2].c_str());
                p.gender = tokens[3];
                p.department = tokens[4];
                p.chestPain = atoi(tokens[5].c_str());
                p.diabetes = atoi(tokens[6].c_str());
                p.bp = atoi(tokens[7].c_str());
                p.cholesterol = atoi(tokens[8].c_str());
                p.bmi = atof(tokens[9].c_str());
                p.maxHR = atoi(tokens[10].c_str());
                p.hypertension = atoi(tokens[11].c_str());
                p.symptoms = tokens[12];
                patients.push_back(p);
            }
        }
        f.close();

        totalPatients = patients.size();
        setColor(COLOR_GREEN);
        std::cout << "✓ Loaded " << totalPatients << " patients\n";
        setColor(COLOR_RESET);

        waitForUser("Press ENTER to start ML triage scoring...");

        // Score patients
        displayHeader("ML TRIAGE SCORING");
        setColor(COLOR_YELLOW);
        std::cout << "Using Random Forest ML model to predict urgency...\n\n";
        setColor(COLOR_RESET);

        for (size_t i = 0; i < patients.size(); i++) {
            double score = getMLScore(patients[i]);
            patients[i].priority = score;

            if (score >= 90) critical++;
            else if (score >= 75) high++;

            // Show ML scoring
            std::cout << "Patient " << std::setw(2) << patients[i].id << ": "
                     << std::setw(20) << std::left << patients[i].name << " -> ";

            if (score >= 90) setColor(COLOR_RED);
            else if (score >= 75) setColor(14); // Dark Yellow
            else if (score >= 60) setColor(COLOR_YELLOW);
            else setColor(COLOR_GREEN);

            std::cout << "Priority: " << std::fixed << std::setprecision(1) << score;

            if (score >= 90) std::cout << " [CRITICAL]";
            else if (score >= 75) std::cout << " [HIGH]";
            else if (score >= 60) std::cout << " [MEDIUM]";
            else std::cout << " [LOW]";

            setColor(COLOR_RESET);
            std::cout << "\n";

            // Add to queue
            if (queues.find(patients[i].department) == queues.end()) {
                queues[patients[i].department] = new PatientQueue();
            }
            queues[patients[i].department]->insert(patients[i]);

            if ((i + 1) % 5 == 0) {
                waitForUser("Press ENTER to continue scoring...");
            }
        }

        std::cout << "\n";
        setColor(COLOR_GREEN);
        std::cout << "✓ All patients scored and added to Fibonacci Heap queues!\n";
        setColor(COLOR_RESET);

        explainFibonacciHeap();
        waitForUser("\nPress ENTER to view queue structure...");
        showQueueDetails();
    }

    void showQueueDetails() {
        displayHeader("FIBONACCI HEAP QUEUE STRUCTURE");

        for (std::map<std::string, PatientQueue*>::iterator it = queues.begin();
             it != queues.end(); ++it) {

            std::string dept = it->first;
            PatientQueue* queue = it->second;

            setColor(COLOR_MAGENTA);
            std::cout << "\n" << dept << " Department Queue (Fibonacci Heap):\n";
            setColor(COLOR_RESET);
            std::cout << "Size: " << queue->size() << " patients\n";

            if (!queue->isEmpty()) {
                std::vector<Patient> all = queue->getAll();
                std::cout << "\nHeap Structure (Max-Heap - Highest Priority First):\n";
                std::cout << "-----------------------------------------------------\n";

                for (size_t i = 0; i < all.size() && i < 10; i++) {
                    std::cout << "  [" << (i+1) << "] ";

                    if (all[i].priority >= 90) setColor(COLOR_RED);
                    else if (all[i].priority >= 75) setColor(14);
                    else if (all[i].priority >= 60) setColor(COLOR_YELLOW);
                    else setColor(COLOR_GREEN);

                    std::cout << std::setw(20) << std::left << all[i].name
                             << " | Priority: " << std::fixed << std::setprecision(1)
                             << all[i].priority;
                    setColor(COLOR_RESET);
                    std::cout << "\n";
                }

                if (all.size() > 10) {
                    std::cout << "  ... and " << (all.size() - 10) << " more patients\n";
                }
            }
        }
    }

    Doctor* findDoctor(const std::string& dept) {
        for (size_t i = 0; i < doctors.size(); i++) {
            if (doctors[i].department == dept && !doctors[i].isBusy) {
                return &doctors[i];
            }
        }
        return NULL;
    }

    int getTreatmentTime(double priority) {
        if (priority >= 90) return 3;
        if (priority >= 75) return 4;
        if (priority >= 60) return 5;
        return 6;
    }

    void showStats() {
        std::cout << "\n";
        setColor(COLOR_CYAN);
        std::cout << "=== CURRENT STATISTICS ===\n";
        setColor(COLOR_RESET);
        std::cout << "Total Patients:    " << totalPatients << "\n";
        setColor(COLOR_GREEN);
        std::cout << "Treated:           " << treated << "\n";
        setColor(COLOR_YELLOW);
        int waiting = 0;
        for (std::map<std::string, PatientQueue*>::iterator it = queues.begin();
             it != queues.end(); ++it) {
            waiting += it->second->size();
        }
        std::cout << "Waiting:           " << waiting << "\n";
        setColor(COLOR_RED);
        std::cout << "Critical Cases:    " << critical << "\n";
        setColor(COLOR_MAGENTA);
        std::cout << "High Priority:     " << high << "\n";
        setColor(COLOR_RESET);
    }

public:
    InteractiveHospital() : totalPatients(0), treated(0), critical(0), high(0), verboseMode(true) {}

    ~InteractiveHospital() {
        for (std::map<std::string, PatientQueue*>::iterator it = queues.begin();
             it != queues.end(); ++it) {
            delete it->second;
        }
    }

    void run() {
        displayHeader("WELCOME TO INTERACTIVE DEMO");

        std::cout << "This demo allows you to step through the hospital triage system.\n";
        std::cout << "You control when to advance to the next step.\n\n";

        explainFibonacciHeap();

        waitForUser("\nPress ENTER to load system data...");

        displayHeader("LOADING SYSTEM DATA");
        loadDoctors();
        Sleep(500);
        loadPatients();

        waitForUser("\nPress ENTER to start patient allocation...");

        allocate();

        displayHeader("DEMO COMPLETE!");
        setColor(COLOR_GREEN);
        std::cout << "\n========================================\n";
        std::cout << "  ALL PATIENTS TREATED SUCCESSFULLY!\n";
        std::cout << "========================================\n\n";
        setColor(COLOR_RESET);

        showStats();

        std::cout << "\n\nThank you for the demo!\n";
        system("pause");
    }

    void allocate() {
        int iteration = 0;

        while (true) {
            iteration++;
            bool anyAction = false;

            displayHeader("ITERATION #" + std::to_string(iteration));

            // Update doctors
            for (size_t i = 0; i < doctors.size(); i++) {
                if (doctors[i].isBusy && doctors[i].timeLeft > 0) {
                    doctors[i].timeLeft--;

                    if (doctors[i].timeLeft == 0) {
                        setColor(COLOR_GREEN);
                        std::cout << "✓ Dr. " << doctors[i].name << " completed treating "
                                 << doctors[i].currentPatient->name << "\n";
                        setColor(COLOR_RESET);

                        treated++;
                        delete doctors[i].currentPatient;
                        doctors[i].currentPatient = NULL;
                        doctors[i].isBusy = false;
                        anyAction = true;
                    }
                }
            }

            // Try to allocate
            for (std::map<std::string, PatientQueue*>::iterator it = queues.begin();
                 it != queues.end(); ++it) {

                std::string dept = it->first;
                PatientQueue* queue = it->second;

                if (!queue->isEmpty()) {
                    Doctor* doc = findDoctor(dept);

                    if (doc != NULL) {
                        Patient p = queue->extractMax();

                        setColor(COLOR_CYAN);
                        std::cout << "\n>>> FIBONACCI HEAP EXTRACT-MAX OPERATION <<<\n";
                        setColor(COLOR_RESET);
                        std::cout << "Extracting highest priority patient from " << dept << " queue...\n";
                        std::cout << "Patient: " << p.name << " | Priority: " << p.priority << "\n";
                        std::cout << "Heap operation: O(log n) time complexity\n";

                        Patient* assignedP = new Patient(p);
                        doc->isBusy = true;
                        doc->currentPatient = assignedP;
                        doc->timeLeft = getTreatmentTime(p.priority);

                        setColor(COLOR_GREEN);
                        std::cout << "\n✓ ALLOCATED: " << assignedP->name
                                 << " -> Dr. " << doc->name << "\n";
                        setColor(COLOR_RESET);
                        std::cout << "Treatment time: " << doc->timeLeft << " seconds\n";

                        anyAction = true;
                        break; // One allocation per iteration for demo
                    }
                }
            }

            // Show current status
            std::cout << "\n";
            setColor(COLOR_YELLOW);
            std::cout << "=== DOCTOR STATUS ===\n";
            setColor(COLOR_RESET);

            for (size_t i = 0; i < doctors.size(); i++) {
                if (doctors[i].isBusy) {
                    setColor(COLOR_RED);
                    std::cout << "Dr. " << doctors[i].name << " (" << doctors[i].department << ") - BUSY\n";
                    setColor(COLOR_RESET);
                    std::cout << "  Treating: " << doctors[i].currentPatient->name
                             << " | Priority: " << doctors[i].currentPatient->priority
                             << " | Time left: " << doctors[i].timeLeft << "s\n";
                } else {
                    setColor(COLOR_GREEN);
                    std::cout << "Dr. " << doctors[i].name << " (" << doctors[i].department << ") - AVAILABLE\n";
                    setColor(COLOR_RESET);
                }
            }

            showStats();

            // Check if done
            int waiting = 0;
            for (std::map<std::string, PatientQueue*>::iterator it = queues.begin();
                 it != queues.end(); ++it) {
                waiting += it->second->size();
            }

            bool anyBusy = false;
            for (size_t i = 0; i < doctors.size(); i++) {
                if (doctors[i].isBusy) {
                    anyBusy = true;
                    break;
                }
            }

            if (waiting == 0 && !anyBusy) {
                break;
            }

            waitForUser("\nPress ENTER for next iteration...");
        }
    }
};

int main() {
    InteractiveHospital system;
    system.run();
    return 0;
}
